...
> **Required for dedup:** the cross-run dedup step commits `state/seen.json` back
> to `main`. In the Routine's **Permissions** tab, enable **Allow unrestricted
> branch pushes** for the `healthtech-digest` repo — otherwise the routine can
> only push `claude/`-prefixed branches and the state won't persist between runs.
